...
test.describe("Негативные сценарии входа (ID 371–376)", () => {
  test("Пустой логин, валидный пароль — ID 371", async ({ page }) => {
    ...
    const isInvalid = await loginPage.loginUsername.evaluate(el => el.validity.valueMissing);
  ...
  test("Валидный логин, пустой пароль — ID 373", async ({ page }) => {
    ...
    await loginPage.loginButton.click();
    const isInvalid = await loginPage.loginPassword.evaluate(el => el.validity.valueMissing);
    expect(isInvalid).toBeTruthy();
  });
  ...
  test("Пустые логин и пароль — ID 372", async ({ page }) => {
    ...
    await loginPage.loginPassword.fill("");
    await loginPage.loginButton.click();
    const loginInvalid = await loginPage.loginUsername.evaluate(el => el.validity.valueMissing);
    const pwdInvalid = await loginPage.loginPassword.evaluate(el => el.validity.valueMissing);
